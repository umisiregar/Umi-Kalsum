    ...
    <div class="card">
        <div class="card-body">
            <p class="card-text">
                ...
                <a href="{{ route('incomming_goods.create') }}">
                ...
                <table class="table table-stripped">
                    ...
                    @foreach ($incomming_goods as $item)
                    <tr>
                        <td>{{ $item->goods->name }}</td>
                        <td>{{ $item->entry_date }}</td>
                        <td>{{ $item->quantity }}</td>
                        <td style="width: 25%">
                        <a href="{{ route('incomming_goods.edit', $item->id) }}">
                            <button class="btn btn-sm btn-warning">Edit</button>
                        </a>
                        <form method="POST" style="display: inline" action="{{ route('incomming_goods') }}">
                            @csrf @method('DELETE')
